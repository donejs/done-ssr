...
assetRegister("dep-cache", function(){
	var assets = this.attr("__renderingAssets");
	var script = document.createElement("script"),
		cache = {},
		addDeps = function(bundleName){
			var deps = loader.getDependencies(bundleName);
			cache[bundleName] = deps;
	...
	assets.concat([normalizedMain]).forEach(addDeps);
	var str = "System.depCache = " + JSON.stringify(cache);
	script.appendChild(document.createTextNode(str));
	return script;

});

setAsBundle(loader.main).then(function(name){
	normalizedMain = name;
});
