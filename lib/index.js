...
exports = module.exports = function(cfg, options){
	cfg = cfg || {};
	...
	new Zone({
		...
		startup = steal.startup();
	});

	var startup = startup.then(function(main){
	...
	SSRStream.prototype.render = function(){
		...
		return startup.then(function(main){
			...
			var zonePlugins = [
				ssrGlobalsZone(dom, request, loader),
				canRouteDataZone,
...
/**
 * @function ssr.dom
 * Specify a function that will be used to create DOM
 */
exports.dom = function(factory){
	if(arguments.length === 0) {
		return makeDom;
	}
	if(typeof factory !== "function") {
		throw new Error("ssr.dom expects a function");
	}

	polyfill.factory = factory;
	return makeDom;
};

function makeDom(){
	var factory = polyfill.factory;
	var dom = factory();
	if(!dom.document) {
		throw new Error("Must return a document");
	}
	return dom;
}
